...
public class TheEpicOfAlexander
{
    ...
    [UserSetting("P2: Only show my own Nisi pass line")] public static bool P2RotsSelfOnly { get; set; } = false;

    [UserSetting("Limit Cut (Alpha Sword / Sheer Force) color")] public static ScriptColor LimitCutColor { get; set; } = new() { V4 = new Vector4(1, 0.2f, 0.2f, 1) };
    [UserSetting("P2 Blue Nisi (Alpha) color")] public static ScriptColor P2Blue { get; set; } = new() { V4 = new Vector4(102 / 255f, 136 / 255f, 187 / 255f, 1) };
    [UserSetting("P2 Orange Nisi (Beta) color")] public static ScriptColor P2Orange { get; set; } = new() { V4 = new Vector4(204 / 255f, 136 / 255f, 102 / 255f, 1) };
    [UserSetting("P2 Purple Nisi (Gamma) color")] public static ScriptColor P2Purple { get; set; } = new() { V4 = new Vector4(85 / 255f, 34 / 255f, 153 / 255f, 1) };
    [UserSetting("P2 Green Nisi (Delta) color")] public static ScriptColor P2Green { get; set; } = new() { V4 = new Vector4(51 / 255f, 85 / 255f, 17 / 255f, 1) };
    [UserSetting("Super Jump & Apocalyptic Ray color")] public static ScriptColor SuperJumpColor { get; set; } = new() { V4 = new Vector4(0.6f, 0.2f, 1, 1) };
    ...
    {
        // 第一个读条是3.9秒
        // PlayerFarestOrder 的距离基准是 Owner 的位置，Owner 为 0 时才退化成 dp.Position
        // 必须把 Owner 设为读条的正义(BJ)，否则会以 (0,0,0) 排序 = 恒定选中场地东南角的人
        var dp = sa.FastDp("超级跳跃", evt.SourceId(), 4200, 10);
    ...
    public void AddLiquidRage(Event evt, ScriptAccessory sa)
    {
        lock (_p1RangePos)
        {
            ...
            {
                0 => new Vector3(-8f, 0, +17.0f),
                1 => new Vector3(-00.0f, 0, -19.0f),
                // 3 => new Vector3(-17.0f, 0, -07.5f),
                6 => new Vector3(+17.0f, 0, -8f),
                7 => new Vector3(-17.5f, 0, -8f),
                _ => Vector3.Zero
            };
    ...
    public void ProteanWaveBoss(Event evt, ScriptAccessory sa)
    {
        ...
        else if (_p1CascadeTimes == 2)
        {
            ...
            {
                0 => new Vector3(+02.0f, 0, -02.0f),
                1 => new Vector3(-02.8f, 0, +00.0f),
                2 => new Vector3(-08.0f, 0, -12.0f),
                3 => new Vector3(-7f, 0, +10.6f),
                4 => new Vector3(-0f, 0, +02.8f),
                5 => new Vector3(+02.8f, 0, +00.0f),
                6 => new Vector3(+12.0f, 0, +08.0f),
                7 => new Vector3(-12.0f, 0, -8.0f),
                _ => Center
            ...
            {
                0 => new Vector3(+04.0f, 0, -04.0f),
                1 => new Vector3(-15.0f, 0, +9.0f),
                2 => new Vector3(-02.0f, 0, -02.0f),
                3 => new Vector3(+00.0f, 0, +02.8f),
                4 => new Vector3(+7.5f, 0, +11.5f),
    ...
    private static readonly List<int> P2StartDir = [4, 0, 6, 2, 3, 5, 7, 1];
    ...
    private void P2PassRot(ScriptAccessory sa, uint state)
    {
        foreach (var player in sa.GetParty())
        {
            ...
            var tid = sa.Data.PartyList[P2PassPair[idx]];
            ...
            if (state == 2)
            {
                // MT ST D1 D3 保持默认连线 H1 H2 D2 D4 就近
                if (idx is 2 or 3 or 5 or 7)
                {
                    List<int> lst = idx < 4 ? [5, 7] : [2, 3];
    ...
    public async void CompressedWater(Event evt, ScriptAccessory sa)
    {
        ...
        {
            case 1:
                // 0 1 不去 2 3 6 7 没雷就去 4 5 只有自己中水才去
                if (myIdx < 2)
                    return;
                if (myIdx is 4 or 5 && sa.Data.Me != evt.TargetId())
                    return;
                wpos = new Vector3(86, 0, 100);
                break;
            case 2:
                // 0 1 4 5 不去 2 3 6 7 没水弱也没雷才去
                if (myIdx is 0 or 1 or 4 or 5)
                    return;
                if (me.HasStatus(2144)) // 水弱
                    return;
                wpos = new Vector3(112.73f, 0.00f, 91.38f);
                break;
            case 3:
                // MT ST 不去 其他人没水弱也没雷才去
                if (myIdx < 2)
                    return;
                if (me.HasStatus(2144)) // 水弱
                    return;
    ...
    public async void CompressedLightning(Event evt, ScriptAccessory sa)
    {
        ...
        switch (_p2LightingTimes)
        {
            case 1:
                // 只有中雷的人去 其他人不动
                if (sa.Data.Me != evt.TargetId())
                    return;
                wpos = new Vector3(104.6f, 0, 85.7f);
                break;
            case 2:
                // 有水弱的人去
                if (!me.HasStatus(2144)) // 水弱
            ...
            case 3:
                // 中雷的和 ST 去 其他人不动
                if (myIdx != 1 && !me.HasStatus(2143))
                    return;
                wpos = new Vector3(94.62f, 0.00f, 105.41f);
                break;
            default:
                return;
        }
    ...
    private bool _p3IconEnabled = false;
    private readonly List<uint> _p3IconTargets = [];

    private void P3Reset()
    {
        _p3IconEnabled = false;
        _p3IconTargets.Clear();
    }
    ...
    // 一运结晶放置点位
    private static readonly List<Vector3> JudgmentCrystalPos =
        [new(105.5f, 0, 82.29f), new(94.5f, 0, 82.29f), new(105.5f, 0, 92.96f), new(94.5f, 0, 92.96f)];
    ...
    public void JudgmentCrystalTips(Event evt, ScriptAccessory sa)
    {
        ...
        for (var i = 0; i < JudgmentCrystalPos.Count; i++)
        {
            var cdp = sa.FastDp($"一运结晶点位-{i}", JudgmentCrystalPos[i], 15000, 0.5f);
            cdp.Delay = 9300;
            cdp.Color = new Vector4(0, 0, 1, 1);
            sa.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Circle, cdp);
        }
    ...
    public void Inception(Event evt, ScriptAccessory sa)
    {
        ...
        if (myIdx < 2)
            ...
            withStack = ((IBattleChara?)sa.Data.Objects.SearchByEntityId(sa.Data.Me))?.HasStatus(1122) ?? false;
        var isLeft = !withStack;
        ...
        if (stack != null)
        {
            dp = sa.FastDp("集团罪", stack.EntityId, 8000, 4, withStack);
            sa.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        }
    ...
    public void LimitCutP3Guide1(Event evt, ScriptAccessory sa)
    {
        ...
        // 不镜像 固定奇数(1357)去西 偶数(2468)去东

        var dp = myIdx switch
        {
            0 => sa.WaypointDp(new Vector3(-13, 0, -13) + Center, 9500),
            1 => sa.WaypointDp(new Vector3(+13, 0, -13) + Center, 9500),
            2 => sa.WaypointDp(new Vector3(-13, 0, +13) + Center, 9500),
            3 => sa.WaypointDp(new Vector3(+13, 0, +13) + Center, 9500),
            4 => sa.WaypointDp(new Vector3(-19, 0, +00) + Center, 3300),
            5 => sa.WaypointDp(new Vector3(+19, 0, +00) + Center, 3300),
            6 => sa.WaypointDp(new Vector3(-19, 0, +00) + Center, 9500),
            7 => sa.WaypointDp(new Vector3(+19, 0, +00) + Center, 9500),
    ...
    public void LimitCutP3Guide2(Event evt, ScriptAccessory sa)
    {
        ...
        {
            case 0:
                dp1 = sa.WaypointDp(new Vector3(-19, 0, +00) + Center, 8600, 6300);
            ...
            case 4:
                ...
                dp2 = sa.WaypointDp(new Vector3(-13, 0, -13) + Center, 4200, 10700);
                break;
            case 5:
                // dp1 = sa.WaypointDp(new Vector3(MathF.Sign(pos.X - 100) * 18 + 100, 0, pos.Z), 10700);
                dp1 = sa.WaypointDp(new Vector3(MathF.Sign(pos.X - 100) * 17 + 100, 0, 100), 10700);
            ...
            case 6:
                ...
                dp2 = sa.WaypointDp(new Vector3(-13, 0, +13) + Center, 4200, 14900);
                break;
            case 7:
                // dp1 = sa.WaypointDp(new Vector3(MathF.Sign(pos.X - 100) * 16 + 100, 0, pos.Z), 4200, 10700);
                dp1 = sa.WaypointDp(new Vector3(MathF.Sign(pos.X - 100) * 16 + 100, 0, MathF.Sign(pos.Z - 100) * 3 +100), 4200, 10700);
    ...
    [ScriptMethod(name: "P3-二运后0041点名开关", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:19025"], userControl: false)]
    public async void IncineratingHeatIconEnable(Event evt, ScriptAccessory sa)
    {
        lock (_p3IconTargets)
        {
            if (_p3IconEnabled) return;
            _p3IconTargets.Clear();
            _p3IconEnabled = true;
        }
    ...
    [ScriptMethod(name: "P3-二运后0041点名指路", eventType: EventTypeEnum.TargetIcon, eventCondition: ["Id:0041"])]
    public void IncineratingHeatIconGuide(Event evt, ScriptAccessory sa)
    {
        lock (_p3IconTargets)
        {
            if (!_p3IconEnabled) return;
            _p3IconTargets.Add(evt.TargetId());
            if (_p3IconTargets.Count != 2) return;

            // 双T右 双奶左 没点名的DPS去南
            var myIdx = sa.MyIndex();
            Vector3 wpos;
            if (myIdx < 2)
                wpos = new Vector3(104.5f, 0, 100);
            else if (myIdx < 4)
                wpos = new Vector3(95.5f, 0, 100);
            else if (!_p3IconTargets.Contains(sa.Data.Me))
                wpos = new Vector3(100, 0, 107);
            else
                return;

            var dp = sa.WaypointDp(wpos, 8000);
            sa.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
        }
    }

    // 加一个方法。TargetIcon 0041，会获得两次。P3-二运后分摊触发后才开启，30s后关闭。
    // 当收到两次后，对没有收到0041的DPS指路{100, 0.00, 107}
    // 两个坦克指路{104.5, 0.00, 100}
    // 两个Heal指路{95.5, 0.00, 100}
    ...
    public void FinalWordEscapeDetection(Event evt, ScriptAccessory sa)
    {
        ...
        var dp = sa.WaypointDp(new Vector3(100, 0, 113), 10000);
        sa.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    }
    ...
    public async void FinalWordContactProhibition(Event evt, ScriptAccessory sa)
    {
        ...
        var dp = sa.WaypointDp(new Vector3(100, 0, 111), 10000);
        sa.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    ...
    public void OrdainedStillness(Event evt, ScriptAccessory sa) => sa.Method.TextInfo("Stop", 4000, true);
    ...
    public void FateCalibrationBeta(Event evt, ScriptAccessory sa)
    {
        ...
        var wpos = myIdx switch
        {
            0 => new Vector3(114.0f, 0, 100.0f),
            1 => new Vector3(093.8f, 0, 083.1f),
            2 => new Vector3(112.0f, 0, 100.0f),
            6 => new Vector3(112.0f, 0, 101.7f),
            _ => new Vector3(112.0f, 0, 098.3f),
        };
